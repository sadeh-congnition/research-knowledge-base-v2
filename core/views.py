    ...
    if request.method == "POST":
        ...
        if name:
            ...
            if request.htmx:
                return render(
                    request,
                    "core/partials/project_list_items.html",
                    {"projects": Project.objects.all().order_by("-created_at")},
                )
...
def project_detail(request, pk):
    ...
    if request.method == "POST":
        ...
        if title:
            ...
            if request.htmx:
                return render(
                    request,
                    "core/partials/node_list_items.html",
                    {
                        "nodes": project.nodes.filter(is_deleted=False).order_by(
                            "-created_at"
                        ),
                        "project": project,
                    },
    ...
    return render(
        request, "core/project_detail.html", {"project": project, "nodes": nodes}
    )
...
def global_search(request):
    ...
    if request.htmx:
        return render(
            request, "core/partials/search_results.html", {"results": results}
        )
