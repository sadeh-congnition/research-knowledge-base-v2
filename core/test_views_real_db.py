...
class TestViewsWithRealDatabases:
    ...
    def test_project_delete_view_with_cascade_effects(self):
        ...
        assert response.status_code == 302 or response.status_code == 200  # HTMX or regular
    ...
    def test_node_operations_with_chroma_integration(self):
        ...
        # Test node delete
        response = client.post(
            reverse("node_delete", kwargs={"pk": node.pk}),
            {"_method": "DELETE"}
        )
        assert response.status_code == 302 or response.status_code == 200
    ...
    def test_question_detail_view_redirects(self):
        ...
        expected_url = reverse("project_detail", args=[node.project.pk]) + f"#question-{question.pk}"
    ...
    def test_global_search_view_with_chroma_integration(self):
        ...
        node1 = services.create_node(project, "Python Programming", "Python is a programming language")
        node2 = services.create_node(project, "JavaScript Development", "JavaScript for web development")
        question1 = services.create_question("Python variables", "Dynamic typing in Python", node1)
        ...
        response = client.get(reverse("global_search") + f"?q=Python&project_id={project.pk}")
        assert response.status_code == 200
    ...
    def test_chroma_browser_view_with_real_data(self):
        ...
        test_collection.add(
            ...
            metadatas=[{"type": "test", "source": "view_test"}, {"type": "test", "source": "view_test"}]
        )
    ...
    def test_htmx_responses_for_partial_updates(self):
        ...
        assert response.status_code == 200, f"Expected HTMX partial response (200), got {response.status_code}"
        assert "HTMX Project" in response.content.decode(), "HTMX response should contain created project content"
        ...
        # HTMX requests should return 200 with partial content, not 302 redirects
        assert response.status_code == 200, f"Expected HTMX partial response (200), got {response.status_code}"
        assert "HTMX Node" in response.content.decode(), "HTMX response should contain created node content"
